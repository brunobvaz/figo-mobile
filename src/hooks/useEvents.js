...
export default function useEvents(filter) {
  ...
  const load = useCallback((page, version, refreshing = false) => {
    ...
    request.promise = (async () => {
      try {
        const result = await editorialService.getLocalEvents({ filter, page });
        ...
        if (generation.current === version) setState(current => ({ ...current, error: failure.message || 'Não foi possível carregar os eventos. Tenta novamente.' }));
      } finally {
