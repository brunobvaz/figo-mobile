...
export function ProductsProvider({ children }) {
  ...
  const cacheProducts = useCallback(items => setProducts(current => { const merged = new Map(current.map(x => [x.id, x])); items.forEach(x => merged.set(x.id, x)); return [...merged.values()]; }), []);
