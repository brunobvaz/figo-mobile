...
**Início → Feiras e eventos** apresenta os eventos da collection `events`, criados no backoffice. A lista apresenta a introdução, os filtros e os eventos, sem o card “Agenda local”. `editorialService` delega em `eventService`, que lê `GET /api/v1/events`; `useEvents` gere filtros, páginas de 20 itens, erros recuperáveis e atualização manual ou ao regressar ao ecrã/primeiro plano. `mockEvents.js` fica apenas como fixture de teste, sem uso na aplicação nem fallback em falhas de rede.
...
“Obter direções”, no detalhe, usa `useEventDirections`: solicita localização em primeiro plano apenas ao tocar, obtém coordenadas atuais com `expo-location` (sem geocodificação inversa, perfil ou localização de pesquisa) e abre Mapas da Apple no iOS ou Google Maps no Android. O serviço de mapas recebe origem e destino e calcula o trajeto, a distância e o tempo de viagem de automóvel; o utilizador pode mudar o modo de transporte no mapa. As coordenadas atuais não são enviadas ao backend nem guardadas no perfil. A distância editorial do evento mantém-se independente deste percurso.

O destino vem exclusivamente de `event.geo` em GeoJSON `[longitude, latitude]`, convertido para a ordem latitude/longitude nos links. Eventos antigos ou com coordenadas inválidas têm o botão desativado. Para `locationSource: 'parish'`, o ecrã identifica o destino como aproximado. Completar concelho/freguesia no backoffice permite obter direções para eventos anteriores.

Se a permissão for recusada, os serviços estiverem desativados ou o GPS não responder em 15 segundos, o ecrã permite repetir ou escolher “Definir origem no mapa”. Nesse caso, o mapa recebe apenas o destino e pode pedir a origem ao utilizador. Falhas ao abrir o mapa são recuperáveis; cliques repetidos são bloqueados durante o pedido. Sair do detalhe ou mudar de evento cancela a abertura pendente. O texto da permissão iOS em `app.config.js` passa a mencionar direções para eventos e será incluído no próximo build nativo.

Contratos dos links: [Map Links da Apple](https://developer.apple.com/library/archive/featuredarticles/iPhoneURLScheme_Reference/MapLinks/MapLinks.html) e [Directions do Google Maps](https://developers.google.com/maps/documentation/urls/get-started#directions-action).

Para testar, usar o backend atualizado e a mesma base do backoffice: criar um evento, abrir a lista, consultar o detalhe, editar e puxar para atualizar, e eliminar e confirmar que desaparece após atualização. Testar também uma imagem carregada, filtros, base vazia, ausência de rede e direções com GPS autorizado/recusado. Num ambiente publicado, atualizar primeiro o backend e depois a app; os eventos existentes ficam disponíveis sem migração.

Testes no backend: `npm test -- tests/publicEvents.test.js tests/adminEvents.test.js tests/eventServiceClient.test.js tests/eventsScreenClient.test.js tests/eventDirectionsClient.test.js tests/activeLocationClient.test.js tests/editorialClient.test.js`.
